...
class BlockEventRepositoryTest {
    ...
    @Test
    fun `record passes the current clock through to the DAO insert`() =
        runTest {
    ...
    @Test
    fun `todayCountFlow asks the DAO for blocks since the start of today`() =
        runTest {
    ...
    @Test
    fun `thisWeekCountFlow uses Monday-start boundary`() =
        runTest {
            val dao = mockk<BlockEventDao>()
    ...
    @Test
    fun `topSurfacesThisWeekFlow passes the default limit when none specified`() =
        runTest {
            val dao = mockk<BlockEventDao>()
            val clock = FixedClock(noonJan14Utc)
            val repo = BlockEventRepository(dao, clock)
            ...
            every { dao.topSurfacesSinceFlow(expectedSince, BlockEventRepository.TOP_SURFACES_DEFAULT_LIMIT) } returns
                flowOf(rows)
    ...
    @Test
    fun `allTimeCountFlow delegates straight through`() =
        runTest {
            val dao = mockk<BlockEventDao>()
            val clock = FixedClock(noonJan14Utc)
            val repo = BlockEventRepository(dao, clock)
